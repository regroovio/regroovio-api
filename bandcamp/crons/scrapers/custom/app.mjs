...
const app = async (event) => {
    ...
    try {
        ...
        const response = { functionName: `bandcamp-cron-${genre}-${process.env.STAGE}`, status: 'Success', message: `Scanned ${albumLinks.length} items.` }
        await slackBot(response);
        return response;
    } catch (error) {
        const response = { functionName: `bandcamp-cron-${genre}-${process.env.STAGE}`, status: 'Error', message: error.message }
        await slackBot(response);
