  ...
  try {
    ...
    for (const individualArtist of individualArtists) {
      if (!isVariousArtist(individualArtist)) {
        const artistData = await spotifySearch(token, `artist:${individualArtist}`, "artist", 20);
        if (artistData) {
    ...
    const albums = await spotifySearchAlbum(token, albumName, artistName, year);
    if (albums.length) {
...
const spotifySearchAlbum = async (token, albumName, artistName, year) => {
  const response = await spotifySearch(token, `album:${albumName}`, "album", 50);
  const albums = []
...
const spotifySearch = async (token, query, type, limit) => {
